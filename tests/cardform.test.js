...
const card={token:'test_card_token_1234567890',issuerId:'123',paymentMethodId:'master',installments:1,email:'buyer@example.com',identificationType:'CPF',identificationNumber:'12345678909'};

async function fixture(t,{paymentStatus='approved',paymentDetail='accredited',challenge=false,quoteExpiresAt=Date.now()+600000}={}){
 const config={...readConfig({}),frontendPath:resolve('frontend'),baseURL:'https://loja.example',publicKey:'TEST-public-key',webhookSecret:'test-webhook-secret',checkoutLimit:100};
 ...
 const gateway={configured:true,cardConfigured:true,verifyCardPaymentIdentity:real.verifyCardPaymentIdentity,verifyIdentity:real.verifyIdentity,
  ...
  async create(){throw Error('legacy checkout must not be called');},async get(){throw Error('legacy order must not be called');}};
 ...
 const prepare=(key=randomUUID(),extra={})=>agent.post('/api/card/prepare').set('Origin',config.baseURL).set('X-CSRF-Token',csrf).set('Idempotency-Key',key).send({items:[{productId:'produto_01',quantity:1}],address,quoteId:'quote-1',...extra});
...
test('CardForm prepare returns server-authoritative products + shipping total',async t=>{
 ...
 assert.equal(result.body.productsTotal,15000);assert.equal(result.body.shipping.priceCents,1191);assert.equal(result.body.total,16191);assert.equal(result.body.amount,'161.91');assert.equal(result.body.status,'pending');assert.match(result.body.orderId,/^[0-9a-f-]{36}$/i);assert.ok(result.body.quoteExpiresAt>Date.now());
...
test('CardForm approved payment uses stable idempotency key and server order amount',async t=>{
 ...
 assert.equal(f.createCalls,1);assert.equal(f.lastKey,paymentKey);assert.equal(paid.body.status,'paid');assert.equal(paid.body.providerOrderId,'12345678901');assert.equal(paid.body.paymentId,'12345678901');assert.equal(f.store.db.prepare('SELECT count(*) n FROM outbox').get().n,1);
...
test('CardForm refuses to create a payment after the bound shipping quote expires',async t=>{
 const f=await fixture(t,{quoteExpiresAt:Date.now()-1});const prepared=await f.prepare().expect(200);await f.pay(prepared.body.orderId).expect(409);assert.equal(f.createCalls,0);
});
...
test('signed payment webhook reconciles CardForm payment through /v1/payments identity',async t=>{
 ...
 await request(f.app).post('/api/webhooks/mercadopago?data.id='+id+'&type=payment').set('x-request-id',requestId).set('x-signature',`ts=${ts},v1=${digest}`).send({id:999,live_mode:false,type:'payment',action:'payment.updated',data:{id}}).expect(200);
