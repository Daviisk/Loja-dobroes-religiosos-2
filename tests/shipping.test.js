...
async function fixture(t){
 ...
 const shipping=createShipping(store,provider),config={...readConfig({}),baseURL:'https://loja.example',frontendPath:resolve('frontend'),webhookSecret:'test-only-secret'};
 let calls=0;const gateway={configured:true,async create(){calls++;return {providerOrderId:'ORDTST'+randomUUID().replaceAll('-','').toUpperCase(),checkoutUrl:'https://www.mercadopago.com.br/checkout/v1/redirect?test=1'};}};
 ...
 return {app,agent,post,quote,store,shipping,provider,get calls(){return calls;}};
}
test('five saints cost exactly R$150 each',()=>{assert.equal(products.length,5);assert(products.every(p=>p.priceCents===15000));});
test('server shipping is included in order, gateway total and immutable address; duplicate clicks reuse order',async t=>{
 const f=await fixture(t),q=await f.quote(),body={items,address,quoteId:q.quoteId},key=randomUUID();
 const a=await f.post('/api/checkout',body,key).expect(200);const b=await f.post('/api/checkout',body,key).expect(200);
 assert.equal(a.body.total,32345);assert.equal(a.body.productsTotal,30000);assert.equal(a.body.shipping.priceCents,2345);assert.equal(b.body.orderId,a.body.orderId);assert.equal(f.calls,1);
 const order=f.store.get(a.body.orderId);assert.deepEqual(order.address,address);assert.equal(order.gatewayPayload.total_amount,'323.45');assert.equal(order.gatewayPayload.items.at(-1).unit_price,'23.45');
 const q2=await f.quote();const c=await f.post('/api/checkout',{...body,quoteId:q2.quoteId}).expect(200);assert.equal(c.body.orderId,a.body.orderId);
});
test('changing address with same request key conflicts, fresh key creates separate delivery',async t=>{
 const f=await fixture(t),q=await f.quote(),body={items,address,quoteId:q.quoteId},key=randomUUID();await f.post('/api/checkout',body,key).expect(200);
 await f.post('/api/checkout',{...body,address:{...address,number:'2'}},key).expect(409);
 const r=await f.post('/api/checkout',{...body,address:{...address,number:'2'}}).expect(200);assert.equal(r.body.address.number,'2');
});
test('unconfigured Correios fails closed; client freight and total are rejected',async t=>{
 const f=await fixture(t),q=await f.quote(),body={items,address,quoteId:q.quoteId};
 for(const extra of [{total:1},{shipping:0},{price:1},{paymentStatus:'paid'}])await f.post('/api/checkout',{...body,...extra}).expect(400);
 await f.post('/api/checkout',{items}).expect(400);
 const off=createShipping(f.store,createCorreios({}));await assert.rejects(off.quote('owner',{items,hash:'x'},address.cep),e=>e.code==='shipping_not_configured');assert.equal(f.calls,0);
});
test('CEP, cart, owner, expiry and quote ID cannot be substituted',async t=>{
 const f=await fixture(t),q=await f.quote(),body={items,address,quoteId:q.quoteId};
 await f.post('/api/checkout',{...body,quoteId:randomUUID()}).expect(409);
 await f.post('/api/checkout',{...body,address:{...address,cep:'20040020'}}).expect(409);
 await f.post('/api/checkout',{...body,items:[{productId:'produto_01',quantity:1}]}).expect(409);
 assert.throws(()=>f.shipping.get('other',q.quoteId,{hash:'x'},address.cep),e=>e.code==='shipping_quote_required');
 f.store.db.prepare('UPDATE shipping_quotes SET expires=0').run();await f.post('/api/checkout',body).expect(409);assert.equal(f.calls,0);
});
