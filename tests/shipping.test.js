...
test('server shipping is included in prepared CardForm order and duplicate request reuses order',async t=>{
 ...
 const order=f.store.get(a.body.orderId);assert.deepEqual(order.address,address);assert.equal(order.gatewayPayload.integration,'cardform');assert.equal(order.total,32345);
