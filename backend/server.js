...
const cleanupOrders=createOrderCleanupRunner(store);cleanupOrders();const cleanupTimer=setInterval(()=>cleanupOrders(),60000);cleanupTimer.unref();
...
for(const signal of ['SIGINT','SIGTERM'])process.on(signal,()=>{clearInterval(cleanupTimer);server.close(()=>{store.close();process.exit(0);});});
