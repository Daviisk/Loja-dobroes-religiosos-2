...
export function createShipping(store,provider){
  return {
    ...
    async quote(owner,cart,cep){
      ...
      await store.removeExpiredShippingQuotes(Date.now());
      const result=[];
      for(const option of options){
        ...
        await store.saveShippingQuote({quoteId:q.quoteId,owner,document:q,expiresAt:q.expiresAt});
        const {cartHash,...publicQuote}=q;result.push(publicQuote);
      }
      return result;
    },
    async get(owner,id,cart,cep,{allowExpired=false}={}){
      const row=typeof id==='string'?await store.getShippingQuote(owner,id):null;
      if(!row)fail(409,'shipping_quote_required','Calcule e selecione o frete antes de continuar.');
      const q=typeof row.document==='string'?JSON.parse(row.document):row.document;
      const expires=Number(row.expires??row.expiresAt);
      if(q.cartHash!==cart.hash||q.cep!==cep||(!allowExpired&&expires<Date.now()))fail(409,'shipping_quote_expired','O carrinho ou CEP mudou, ou a cotação expirou. Calcule o frete novamente.');
      return q;
    }
  };
}
