(()=>{
 ...
 async function update(){
  ...
  try{
   ...
   get('#order-message').textContent=messageFor(order);renderProgress(order);const cancel=get('#cancel-order');if(cancel)cancel.hidden=Boolean(order.status==='paid'||['preparing','shipped','delivered','cancelled'].includes(fulfillment)||['failed','refunded','charged_back'].includes(order.status));
 ...
 const cancel=get('#cancel-order');if(cancel)cancel.addEventListener('click',async()=>{if(!token||!confirm('Cancelar este pedido? Esta ação não poderá ser desfeita.'))return;cancel.disabled=true;try{const response=await fetch('/api/cancel-order?orderId='+encodeURIComponent(id)+'&token='+encodeURIComponent(token),{method:'POST',credentials:'same-origin',cache:'no-store',signal:AbortSignal.timeout(15000)});const data=await response.json();if(!response.ok)throw Error(data.message||'Não foi possível cancelar o pedido.');polls=0;await update();}catch(error){get('#order-message').textContent=error.message;}finally{cancel.disabled=false;}});
